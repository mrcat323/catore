<?php
    ...
    public static function addOrder($name, $email, $address, $message) {
        $db = new Database();
        return $db->query("INSERT INTO `orders` SET `name` = ?, `email` = ?, `address` = ?, `message` = ?, `order_date` = UNIX_TIMESTAMP()", [$name, $email, $address, $message]);
    }
